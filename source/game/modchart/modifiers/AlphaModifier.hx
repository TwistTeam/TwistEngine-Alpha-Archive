...
class AlphaModifier extends NoteModifier
{
	...
	override function updateNote(beat:Float, note:Note, pos:Vector3, player:Int)
	{
		...
		var alphaMod = (1.0 - getSubmodValue("alpha", player))
				* (1.0 - getSubmodValue('alpha${note.noteData}', player))
				* (1.0 - getSubmodValue("noteAlpha", player))
				* (1.0 - getSubmodValue('noteAlpha${note.noteData}', player));
		var alpha = 1.0 - getVisibility(yPos, player, note);

		if (getSubmodValue("dontUseStealthGlow", player) == 0)
		{
			note.colorTransform.alphaOffset = -getAlpha(alpha);
			// note.alphaMod = getAlpha(alpha);
			// note.colorSwap.flash = getGlow(alpha);
			note.colorTransform.redOffset = note.colorTransform.greenOffset = note.colorTransform.blueOffset = (1.0 - getGlow(alpha)) * note.colorTransform.alphaOffset;
		}
		else
		{
			note.colorTransform.alphaOffset = -alpha;
			note.colorTransform.redOffset = note.colorTransform.greenOffset = note.colorTransform.blueOffset = 0.0;
		}
		note.colorTransform.alphaOffset *= alphaMod;
		// trace(getSubmodValue("dontUseStealthGlow", player), note.colorTransform.alphaOffset, alphaMod, alpha);
	}
	...
	override function updateReceptor(beat:Float, receptor:StrumNote, pos:Vector3, player:Int)
	{
		...
		{
			alpha = (1.0 - alpha) * (1.0 - getSubmodValue("dark", player)) * (1.0 - getSubmodValue('dark${receptor.noteData}', player));
		}
		receptor.colorTransform.alphaOffset = -255.0 * (1.0 - getAlpha(alpha));
		// @:privateAccess
