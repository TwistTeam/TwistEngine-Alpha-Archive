...
class ReverseModifier extends NoteModifier
{
	...
	*/
	...
	override function getPos(time:Float, visualDiff:Float, timeDiff:Float, beat:Float, pos:Vector3, data:Int, player:Int, obj:INote)
	{
		...
		*/
		var receptor:StrumNote = obj is StrumNote ? cast obj : obj is Note ? cast (obj, Note).parentStrum : null;
		if (receptor == null)
			return pos;

		var perc = getReverseValue(data, player);
		if (receptor.getDownScroll())
			perc = 1.0 - perc;
		var mult = CoolUtil.scale(perc, 0, 1, 1, -1);
		var shift = CoolUtil.scale(perc, 0, 1, receptor.defPos.y, FlxG.height - receptor.defPos.y - Note.swagWidth / 2);
		shift = CoolUtil.scale(getSubmodValue("centered", player), 0, 1, shift, FlxG.height / 2 - Note.swagWidth / 2);
		pos.y = shift + visualDiff * mult;
