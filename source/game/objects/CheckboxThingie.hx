...
class CheckboxThingie extends FlxSprite
{
	...
	public var scalebase:Float = 0;

	public function new(x:Float = 0, y:Float = 0, scalebase:Float = 1, ?checked = false)
	{
		super(x, y);
		...
		setGraphicSize(0.9 * width * scalebase);
		updateHitbox();
	...
	private function animationFinished(name:String)
	{
		switch (name)
		{
			case 'checking':
				...
				offset.set(3 * scalebase, 12 * scalebase);

			case 'unchecking':
				animation.play('unchecked', true);
				offset.set(0, 2 * scalebase);
		}
	}
}
