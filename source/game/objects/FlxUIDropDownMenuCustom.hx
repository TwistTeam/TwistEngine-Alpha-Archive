...
class FlxUIDropDownMenuCustom extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
	 ...
	 */
	public function setData(DataList:Array<StrNameLabel>, updateShit:Bool = true):Void
	{
		var i:Int = 0;
		...
		if (DataList != null)
		{
			...
			if(updateShit == true) selectSomething(DataList[0].name, DataList[0].label);
		}
	...
	public override function update(elapsed:Float):Void{
		...
		if (dropPanel.visible){
			...
			if (FlxG.mouse.justPressed && !CoolUtil.mouseOverlapping(this))
				showList(false);
		}
		#end
	}
	...
	private function showList(b:Bool):Void{
		...
		dropPanel.visible = b;
		if(currentScroll > list.length - 1){
...
class FlxUIDropDownHeader extends FlxUIGroup
{
	 ...
	 */
	public function new(Width:Int = 120, Height:Int = 20, ?Background:FlxSprite, ?Text:FlxUIText, ?Button:FlxUISpriteButton)
	{
		super();
		...
		if (background == null)
		{
			background = new FlxUI9SliceSprite(0, 0, FlxUIAssets.IMG_BOX, new Rectangle(0, 0, Width, Height), [1, 1, 14, 14]);
		}
