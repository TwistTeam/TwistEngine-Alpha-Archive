...
import game.objects.Alphabet.TypedAlphabet;
...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		box.visible = false;
		box.setGraphicSize(box.width * 0.9);
		box.updateHitbox();
		add(box);
	...
	function spawnCharacters() {
		...
		for (individualChar in charsMap.keys()) {
			...
			char.setGraphicSize(char.width * DialogueCharacter.DEFAULT_SCALE * char.jsonFile.scale);
			char.updateHitbox();
