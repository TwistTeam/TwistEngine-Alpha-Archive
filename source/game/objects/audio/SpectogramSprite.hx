...
import game.backend.system.audio.visualize.VisShit;
import game.backend.system.audio.visualize.VisShit.VISTYPE;
import game.backend.system.audio.visualize.VisShit.CurAudioInfo;
import game.backend.system.audio.visualize.dsp.FFT;
...
class SpectogramSprite extends FlxTypedSpriteGroup<FlxSprite>
{
	...
	public function generateSection(start:Float = 0, seconds:Float = 1):Void
	{
		...
		if (setBuffer)
		{
			...
			var prevLine:FlxPoint = FlxPoint.get();
			...
			for (i in 0...group.members.length)
			{
				...
				var line = FlxPoint.get(prevLine.x - obj.x, prevLine.y - obj.y);

				obj.setGraphicSize(Math.max(line.length, 1), 1);
				obj.angle = line.degrees;
				line.put();
			}
			prevLine.put();
		...
		if (vis.snd != null)
		{
			...
			if (setBuffer && doAnim)
			{
				...
				for (i in 0...group.members.length)
				{
					...
					// obj.setGraphicSize(Std.int(Math.max(line.length, 1)), Std.int(1));
					// obj.angle = line.degrees;
					// line.put();
				}
				prevLine.put();
		...
		if (vis.snd != null)
		{
			...
			if (setBuffer)
			{
				...
				for (sample in remappedShit...remappedShit + lengthOfShit)
				{
					...
					var obj = group.members[Std.int(remappedSample)];
					obj.x = prevLine.x;
					obj.y = prevLine.y;
