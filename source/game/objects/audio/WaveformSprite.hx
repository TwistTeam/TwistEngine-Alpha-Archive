...
import game.backend.system.audio.waveform.WaveformData;
import game.backend.system.audio.waveform.WaveformDataParser;
...
class WaveformSprite extends MeshRender
{
	...
	function drawWaveform():Void
	{
		...
		var _size = (orientation == HORIZONTAL ? this.height : this.width);
		if (pixelsPerIndex >= 1.0)
		{
			...
			for (i in startIndex...endIndex)
			{
				...
				var sampleMax:Float = Math.min(channel.maxSampleMapped(i) * amplitude, 1.0);
				var sampleMin:Float = Math.max(channel.minSampleMapped(i) * amplitude, -1.0);
				var sampleMaxSize:Float = sampleMax * _size / 2;
				if (sampleMaxSize < minWaveformSize) sampleMaxSize = minWaveformSize;
				var sampleMinSize:Float = sampleMin * _size / 2;
		...
		else
		{
			...
			for (i in 0...waveformLengthPixels)
			{
				...
				var channel = waveformData.channel(0);
				var sampleMax:Float = Math.min(channel.maxSampleRangeMapped(rangeStart, rangeEnd) * amplitude, 1.0);
				var sampleMin:Float = Math.max(channel.minSampleRangeMapped(rangeStart, rangeEnd) * amplitude, -1.0);
				var sampleMaxSize:Float = sampleMax * _size / 2;
				if (sampleMaxSize < minWaveformSize) sampleMaxSize = minWaveformSize;
				var sampleMinSize:Float = sampleMin * _size / 2;
				if (sampleMinSize > -minWaveformSize) sampleMinSize = -minWaveformSize;
	...
	{
		return buildFromWaveformData(WaveformDataParser.interpretFlxSound(sound), orientation, color, duration);
	}
}
