...
class PolygonSpectogram extends MeshRender
{
	...
	public function new(?daSound:FlxSound, ?col:FlxColor, ?height:Float = 720, ?detail:Float = 1)
	{
		super(0, 0, col);
	...
	public function generateSection(start:Float = 0, seconds:Float = 1):Void
	{
		...
		if (setBuffer)
		{
			...
			for (i in 0...funnyPixels)
			{
				...
				build_quad(prevPoint.x, prevPoint.y,
					prevPoint.x + thickness, prevPoint.y,
					coolPoint.x, coolPoint.y,
					coolPoint.x + thickness, coolPoint.y + thickness);
