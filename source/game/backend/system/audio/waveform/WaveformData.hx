package game.backend.system.audio.waveform;
...
class WaveformData
{
	...
	public function new(?version:Int, channels:Int, sampleRate:Int, samplesPerPoint:Int, bits:Int, length:Int, data:Vector<Int>)
	{
	...
	public function resample(newSamplesPerPoint:Int):WaveformData
	{
		...
		if (ratio == 1)
			return result;
		if (ratio < 1)
			trace('[WARNING] Downsampling will result in a low precision.');
	...
	public function merge(that:WaveformData):WaveformData
	{
		...
		var result = this.clone(new Vector<Int>());
		...
		for (channelIndex in 0...this.channels)
		{
			...
			{
				resultChannel.setMinSample(index, Std.int(Math.min(thisChannel.minSample(index), thatChannel.minSample(index))));
				resultChannel.setMaxSample(index, Std.int(Math.max(thisChannel.maxSample(index), thatChannel.maxSample(index))));
			}
		}
	 ...
	 */
	public function clone(?newData:Vector<Int>):WaveformData
	{
		if (newData == null)
		{
