package game.backend.system.audio.visualize;
...
class VisShit
{
	...
	{
		static var _staticCurAudioInfo = { left: 0.0, right: 0.0, balanced: 0.0 } // Init static handler
		_staticCurAudioInfo.left = aud[index] / 32767;
		_staticCurAudioInfo.right = aud[index + 2] / 32767;
		_staticCurAudioInfo.balanced = (_staticCurAudioInfo.left + _staticCurAudioInfo.right) / 2;
		return _staticCurAudioInfo;
	}
	...
	public function checkAndSetBuffer()
	{
		if (snd != null && snd.playing)
		{
			if (!setBuffer)
			{
				// Math.pow3
				@:privateAccess
				var buf = snd._channel.__sound.__buffer;
