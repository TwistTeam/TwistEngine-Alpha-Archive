...
class FileUtil
{
	...
	public static final FILE_FILTER_LUA:FileFilter = new FileFilter("Lua Script File (.lua)", "lua");
	public static final FILE_FILTER_HX:FileFilter = new FileFilter("Hx Script File (.hx)", "hx");
			...
			?dialogTitle:String):Bool
	{
		...
		if (onCancel != null)
			fileDialog.onCancel.add(onCancel);
		fileDialog.browse(SAVE, filter, defaultPath.replace('/', '\\') , dialogTitle);
		return true;
		#else
		onCancel();
		return false;
	...
	{
		#if (desktop || html5)
		var filter:String = #if html5
				defaultFileName != null ? Path.extension(defaultFileName) : null;
			#else
	...
	public static function moveDir(path:String, destination:String, ?ignore:Array<String>):Void
	{
		...
		{
			if (ignore.contains(Path.join([path, entry])))
				continue;
	...
	public static function deleteDir(path:String, recursive:Bool = false, ?ignore:Array<String>):Void
	{
		...
		if (recursive)
		{
			...
			for (entry in readDir(path))
			{
				...
				isDirectory(entryPath) ? deleteDir(entryPath, true, ignore) : deleteFile(entryPath);
			}
	...
	public static function getDirSize(path:String):Int
	{
		...
		{
			var entryPath = Path.join([path, entry]);
			total += (isDirectory(entryPath) ? getDirSize(entryPath) : getFileSize(entryPath));
		}
	...
	static function convertTypeFilter(typeFilter:Array<FileFilter>):String
	{
		...
		{
			filters.push(type.extension.replace(';', ','));
		}

		return filters.join(';');
	}

	public static function getPathFromCurrentRoot(attributes:Array<String>):String
	{
		var attributes:Array<String> = attributes.copy();
		attributes.unshift(#if MODS_ALLOWED ModsFolder.currentModFolderPath #else "assets" #end);
		var path = Path.join(attributes);

		#if sys
		if (!sys.FileSystem.exists(path.substring(0, path.lastIndexOf("/")) + "/"))
		{
			attributes[0] = "assets";
...
class FileUtilSandboxed
{
	...
	public static final FILE_FILTER_LUA:FileFilter = FileUtil.FILE_FILTER_LUA;
	...
	public static function saveFilesAsZIPToPath(resources:Array<Entry>, path:String, mode:FileWriteMode = Skip):Bool
	{
		if (isProtected(path = sanitizePath(path)))
			return false;
	...
	{
		if (isProtected(path = sanitizePath(path)))
			throw 'Cannot write to protected path: $path';
	...
	{
		if (isProtected(path = sanitizePath(path)))
			throw 'Cannot move protected path: $path';
		if (isProtected(destination = sanitizePath(destination)))
			throw 'Cannot move to protected path: $destination';
	...
	{
		if (isProtected(path = sanitizePath(path)))
			throw 'Cannot delete protected path: $path';
	...
	{
		if (isProtected(path = sanitizePath(path)))
			throw 'Cannot rename protected path: $path';
