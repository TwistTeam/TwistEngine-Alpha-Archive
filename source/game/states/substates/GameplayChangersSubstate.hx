...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	function getOptions()
	{
		...
		option = new GameplayOption('Playback Rate', 'songspeed', 'float', 1);
		option.scrollSpeed = 1;
		#if DEV_BUILD
		option.minValue = 0.1;	// real
		option.maxValue = 10.0;	// real
		#else
		option.minValue = 0.5;	// real
		option.maxValue = 2.0;	// real
		#end
		...
		option = new GameplayOption('Health Drain', 'healthdrainperc', 'percent', 0);
		option.changeValue = 0.05;
		option.maxValue = 2;
		optionsArray.push(option);
		...
		*/
		...
		option = new GameplayOption('Health Gain Multiplier', 'healthgain', 'float', 1);
		...
		option = new GameplayOption('Health Loss Multiplier', 'healthloss', 'float', 1);
		option.scrollSpeed = 2.5;
		option.minValue = 0.5;
		option.maxValue = 5;
		option.changeValue = 0.1;
	...
	public function new()
	{
		...
		var bg:FlxBGSprite = new FlxBGSprite();
		bg.alpha = 0.6;
		bg.color = 0xff000000;
		bg.scrollFactor.set();
		add(bg);
		...
		for (i => data in optionsArray)
		{
			var optionText:Alphabet = new Alphabet(200, 360, data.name, true);
			...
			if(data.type == 'bool') {
				...
				var checkbox:CheckboxThingie = new CheckboxThingie(optionText.x - 105, optionText.y, data.getValue() == true);
				...
				optionText.snapToPosition();
				var valueText:AttachedText = new AttachedText(Std.string(data.getValue()), optionText.width + 40, 0, false, 0.8, "PhantomMuff Full Letters 1-1-5.ttf");
				valueText.sprTracker = optionText;
				valueText.copyAlpha = true;
				valueText.ID = i;
				valueText.color = 0xffffffff;
				// valueText.color = 0xff000000;
				valueText.scrollFactor.set();
				valueText.setBorderStyle(FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				valueText.borderSize *= 3;
				grpTexts.add(valueText);
				data.setChild(valueText);
			}
			updateTextFrom(data);
		}
					...
					if (holdTime > 0.5 || pressed)
					{
						if (pressed){
							...
							switch (curOption.type){
								...
								case 'string':
									var num:Int = FlxMath.wrap(curOption.curOption + (controls.UI_LEFT_P ? -1 : 1),
																0, curOption.options.length - 1);
							...
							switch (curOption.type){
								...
								case 'float' | 'percent':
									curOption.setValue(FlxMath.roundDecimal(Math.round(holdValue / curOption.changeValue) * curOption.changeValue, curOption.decimals));
							}
							updateTextFrom(curOption);
							curOption.change();
						}
	...
	function changeSelection(change:Int = 0)
	{
		...
		var bullShit:Int = -curSelected;

		for (item in grpOptions.members)
		{
			item.targetY = bullShit++;
			item.alpha = 0.6;
		}
...
{
	private var child:AttachedText;
	...
	public function setChild(child:AttachedText)
		this.child = child;
