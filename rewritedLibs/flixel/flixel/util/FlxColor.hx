...
abstract FlxColor(Int) from Int from UInt to Int to UInt
{
	...
	public static function fromString(str:String):Null<FlxColor>
	{
		...
		else
		{
			...
			if (colorLookup.exists(str))
				result = new FlxColor(colorLookup.get(str));
			/*
	...
	public inline function getAnalogousHarmony(Threshold:Int = 30):Harmony
	{
		return {
			original: this,
			warmer: fromHSB(FlxMath.wrap(Std.int(hue) - Threshold, 0, 360), saturation, brightness, alphaFloat),
			colder: fromHSB(FlxMath.wrap(Std.int(hue) + Threshold, 0, 360), saturation, brightness, alphaFloat)
		};
	}
	...
	{
		var oppositeHue:Int = FlxMath.wrap(Std.int(hue) + 180, 0, 360);
		return {
			original: this,
			warmer: fromHSB(FlxMath.wrap(oppositeHue - Threshold, 0, 360), saturation, brightness, alphaFloat),
			colder: fromHSB(FlxMath.wrap(oppositeHue + Threshold, 0, 360), saturation, brightness, alphaFloat)
		};
	}
	...
	function get_hue():Float
	{
		...
		var hue:Float = 180 / Math.PI * hueRad;
