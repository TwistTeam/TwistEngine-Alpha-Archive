...
class FlxMouseEventManager extends FlxBasic
{
	 ...
	 */
	public var limitedTouchesCount:Int = -1;
	...
	public override function update(elapsed:Float):Void
	{
		...
		for (current in currentOverObjects)
		{
			if (current.onMouseDown != null && current.object.exists && current.object.visible)
			{
				...
				{
					if (FlxMouseButton.getByID(buttonID).justPressed #if FLX_TOUCH || (FlxG.touches.getByID(0) != null && FlxG.touches.getByID(0).justPressed) #end)
					{
	...
	{
		addEvent(new FlxMouseEvent<T>(object, onMouseDown, onMouseUp, onMouseOver, onMouseOut, mouseChildren, mouseEnabled, pixelPerfect, mouseButtons));
		return object;
	}
	...
	function checkOverlap<T:FlxObject>(event:FlxMouseEvent<T>):Bool
	{
        #if FLX_TOUCH
		var touch;
		var totalTouchesLength = limitedTouchesCount == -1 ? FlxG.touches.list.length : FlxMath.minInt(limitedTouchesCount, FlxG.touches.list.length);
		#end
		for (camera in event.object.cameras)
		{
            if (camera.alive && camera.exists)
            {
                #if FLX_MOUSE
				...
				if (totalTouchesLength > 0)
				{
                	for (i in 0...totalTouchesLength)
                	{
                	    ...
                	    touch.getPositionInCameraView(camera, _point);
                	    if (camera.containsPoint(_point))
                	    {
