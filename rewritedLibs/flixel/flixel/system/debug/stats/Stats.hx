...
class Stats extends Window
{
	...
	#if hxvlc
	var deVideo:FlxVideo;
	var stupidFill:Bitmap;
	#end
	...
	public function new()
	{
		...
		if (Assets.exists('assets/videos/badapplelol.mp4'))
		{
			...
			deVideo.resizeMode = NONE;
			@:privateAccess deVideo.x = fpsGraph._width / 2 + 1;
			deVideo.y = -5 + 1;
			stupidFill.x = deVideo.x;
			...
			deVideo.onFormatSetup.add(() -> {
				@:privateAccess stupidFill.width = deVideo.width = fpsGraph._width * 1.1;
				stupidFill.height = deVideo.height = graphHeight + 10;
			});
			fpsGraph.addChildAt(deVideo, 1);
			fpsGraph.addChildAt(stupidFill, 2);
		}
		#end
