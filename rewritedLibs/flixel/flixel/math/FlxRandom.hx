...
class FlxRandom
{
	...
	{
		return initialSeed = Std.int(Math.random() * FlxMath.MAX_VALUE_INT);
	}
	...
	public function getObject<T>(Objects:Array<T>, ?WeightsArray:Array<Float>, StartIndex:Int = 0, ?EndIndex:Null<Int>):T
	{
		...
		if (Objects.length != 0)
		{
			...
			selected = Objects[StartIndex + weightedPick([for (i in StartIndex...EndIndex + 1) WeightsArray[i]])];
		}
	...
	public function shuffleArray<T>(Objects:Array<T>, HowManyTimes:Int):Array<T>
	{
		...
		{
			pick1 = int(0, Objects.length - 1);
			pick2 = int(0, Objects.length - 1);
