...
class FlxGame extends Sprite
{
	 ...
	 */
	public var soundTray(default, null):IFlxSoundTray;
	#end
	 ...
	 */
	var _flixelFilters:Array<BitmapFilter>;
	 ...
	 */
	var _customSoundTray:Class<IFlxSoundTray> = FlxSoundTray;
	#end
	...
	function create(_):Void
	{
		...
		// Volume display tab
		#if FLX_SOUND_TRAY
		replaceSoundTray(_customSoundTray);
		#end
	 ...
	 */
	public function replaceSoundTray(?newClassSoundTray:Class<IFlxSoundTray>, ?args:Array<Dynamic>, ?newObjSoundTray:IFlxSoundTray)
	{
		if (newClassSoundTray != null)
			newObjSoundTray ??= Type.createInstance(newClassSoundTray, args ?? []);
		newObjSoundTray.onAdded(this, soundTray);
		soundTray?.dispose();
		soundTray = newObjSoundTray;
		// this.soundTray = replaceObject(_customSoundTray, null, this.soundTray, newSoundTray);
	}
	#end
	 ...
	 * @since 5.6.2-TWIST-PSYCH
	 */
	public function replaceLostScreen(?focusLostScreen:FlxFocusLostScreen) // TODO: Interface for FlxFocusLostScreen
	{
		this._focusLostScreen = replaceObject(_customFocusLostScreen, null, this._focusLostScreen, focusLostScreen);
	}
	#end
	 ...
	 * @since 5.6.2-TWIST-PSYCH
	 */
	public function replaceObject<T:openfl.display.DisplayObject>(?classRef:Class<T>, ?args:Array<Dynamic>, ?origObj:T, ?overrideObj:T):T
	{
		var index:Int = origObj == null ? numChildren : getChildIndex(origObj);
		removeChild(origObj);
		if (classRef != null)
		{
			overrideObj ??= Type.createInstance(classRef, args ?? []);
			addChildAt(overrideObj, index);
		}
		return overrideObj;
	}
	...
	public var enableFixedDraw:Bool = false; // WIP
	var lastFT:Float = 0;
	/**
