...
#if !macro
import game.backend.utils.Types.UByte;
#end
...
class FlxSprite extends FlxObject
{
	...
	/**
	 * Texture filter for drawing objects with `antialiasing` enabled.
	 * EXPERIMENTALLY
	 * Default is `LINEAR`.
	 * @since 5.6.2-TWIST-PSYCH-TEST
	*/
	public static var defaultTextureFilter:Context3DTextureFilter = LINEAR;

	/**
	 * MipMap filter for drawing objects with `antialiasing`' enabled.
	 * EXPERIMENTALLY
	 * Default is `MIPNONE`.
	 * @since 5.6.2-TWIST-PSYCH-TEST
	*/
	public static var defaultMipFilter:Context3DMipFilter = MIPLINEAR;

	/**
	 * Lod bias for rendering objects with mipmap enabled.
	 * EXPERIMENTALLY
	 * Default is `-0.2`.
	 * @since 5.6.2-TWIST-PSYCH-TEST
	*/
	public static var defaultLodBias:Float = -0.2;

	/**
	 * Allows to dynamically use antialiasing in rendering calls.
	 * Default is `true`.
	...
	*/
	 ...
	 * @since 5.6.2-TWIST-PSYCH
	 */
	public static var defaultHaxeFlixelLogo:FlxGraphicAsset = "flixel/images/logo/default.png"; // TODO: Move it in to FlxAssets
	 ...
	 */
	public var useFramePixels(default, set):Bool;
	...
	public function updateFramePixels():BitmapData
	{
		...
		{
			framePixels = _frame.paint(framePixels, null, false, true);
		}
		else
		{
			framePixels = _frame.paintRotatedAndFlipped(framePixels, null, FlxFrameAngle.ANGLE_0, doFlipX, doFlipY, false, true);
		}
	...
	{
		_facingFlip.set(Direction, new FlxSpriteFacingFlip(FlipX, FlipY));
	}
	...
	{
		if (_facingFlip.exists(Direction))
		{
			var flip:FlxSpriteFacingFlip = _facingFlip.get(Direction);
			flipX = flip.x;
			flipY = flip.y;
		}
...
abstract FlxSpriteFacingFlip(#if macro Int #else UByte #end)
	#if !macro from UByte to UByte #end
	// from FlxSpriteFacingFlipDynamic to FlxSpriteFacingFlipDynamic
	from Int to Int
{
