...
class FlxClothSprite extends FlxSprite
{
	 ...
	 */
	public function setMesh(columns:Int = 0, rows:Int = 0, meshPixelsWidth:Int = 0, meshPixelsHeight:Int = 0, ?pinned:Array<Int>):Void
	{
	...
	function updatePoints(elapsed:Float)
	{
		...
		for (p in points)
		{
			if (!p.pinned)
			{
				vx = (p.x - p.oldx) * meshFriction.x;
				vy = (p.y - p.oldy) * meshFriction.y;
	...
	{
		var dx, dy, distance, difference, offsetX, offsetY;
		for (s in constraints)
		{
			dx = s.p1.x - s.p0.x;
			dy = s.p1.y - s.p0.y;
			...
			offsetX = dx * 0.5 * difference;
			offsetY = dy * 0.5 * difference;
