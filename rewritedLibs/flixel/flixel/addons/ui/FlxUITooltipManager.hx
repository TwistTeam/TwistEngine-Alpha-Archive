...
class FlxUITooltipManager implements IFlxDestroyable
{
	...
	public function update(elapsed:Float):Void
	{
		...
		for (i => data in list)
		{
			...
			if (data.sticky == false && (false == btn.visible || btn.justMousedOut || btn.mouseIsOut))
			{
				data.count = 0;
				hide(i);
			}
			...
			if (data.count > delay || (data.data.delay >= 0 && data.count > data.data.delay))
			{
				if (current != i)
				{
					show(i);
				}
				else if (data.data.moving)
				{
					show(i);
				}
			}
