...
class FlxInputText extends FlxText
{
			...
			BackgroundColor:Int = FlxColor.WHITE, EmbeddedFont:Bool = true)
	{
		...
		game.backend.utils.CoolUtil.removeEventListeners(textField);
	...
	{
		if (Sprite == null || !Sprite.visible || Sprite.alpha == 0)
			return;
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (FlxG.mouse.justPressed)
		{
			...
			if (CoolUtil.mouseOverlapping(backgroundSprite, camera))
			{
	 ...
	 */
	private function onKeyDown(e:KeyCode, modifier:KeyModifier):Void
	{
		if (hasFocus)
		{
			switch (e)
			{
				case TAB: // tab doesn't work in textfield, sorry!
				// 	trace("TAB");
					// onTextInput("\t");
	...
	private function onTextInput(newText:String):Void
	{
		if (hasFocus)
		{
			...
			#if (html5 && js)
			@:privateAccess if (Clipboard._text == newText)
				pasteClipboardText(newText);
			#end
		}
	...
	{
		var newText:String = filter(clipboardText);
		if (newText.length > 0 && (maxLength == 0 || (text.length + newText.length) < maxLength)) {
			text = insertSubstring(text, newText, caretIndex);
	...
	{
		#if FLX_MOUSE
		// var thisPos = getScreenPosition();
		var result = getCharIndexAtPoint(FlxG.mouse.screenX - x, FlxG.mouse.screenY - y);
		// thisPos.put();
		return result;
		#else
		return 0;
	...
	private override function set_text(Text:String):String
	{
		...
		for (i in 0...numChars)
		{
			textField.appendText(Text.charAt(i)); // add a character
	...
	private function prepareCharBoundaries(numChars:Int):Void
	{
		...
		{
			for (i in _charBoundaries.length - 1...numChars)
			{
	...
	static var _lastShit:Null<Bool> = null;
	private function set_hasFocus(newFocus:Bool):Bool
	{
		...
		{
			var newLast = _lastShit == null;
			if (_lastShit != true)
				_lastShit = newFocus;
			if (newLast)
				FlxG.signals.postUpdate.addOnce(() -> {
					FlxG.sound.keysAllowed = !_lastShit;
					_lastShit = null;
				});
