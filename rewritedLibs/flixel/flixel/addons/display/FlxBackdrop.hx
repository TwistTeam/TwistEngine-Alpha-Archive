...
class FlxBackdrop extends FlxSprite
{
	...
	override function drawComplex(camera:FlxCamera)
	{
		...
		// for (tileX in 0...tilesX)
		{
			...
			// for (tileY in 0...tilesY)
			{
				_tileMatrix.tx = _matrix.tx;
				_tileMatrix.ty = _matrix.ty;
				_tileMatrix.translate(tileSize.x * tileX, tileSize.y * tileY);

				if (isPixelPerfect)
				{
					_tileMatrix.tx = Math.ffloor(_tileMatrix.tx);
					_tileMatrix.ty = Math.ffloor(_tileMatrix.ty);
				}
	...
	{
		return value - Math.ffloor((value - min) / step) * step;
	}

	function modMax(value:Float, step:Float, max:Float)
	{
		return value - Math.fceil((value - max) / step) * step;
	}
