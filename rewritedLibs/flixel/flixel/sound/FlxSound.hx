...
class FlxSound extends FlxBasic
{
	 ...
	 */
	public var useTimeScaleToPitch(default, set):Bool = true;
	#end
	...
	public override function update(elapsed:Float):Void
	{
		...
		if (_target != null)
		{
			...
			if (_proximityPan)
			{
				_transform.pan = FlxMath.bound((x - _target.x) / _radius, -1, 1);
			}
		}
		...
		if (_transform.volume > 0)
		{
			...
			amplitude = (amplitudeLeft + amplitudeRight) / 2.0;
		}
		else
		{
			amplitudeLeft = 0;
	...
	{
		return playing ? _time = _channel.position : _time;
	}
	...
	function init(Looped:Bool = false, AutoDestroy:Bool = false, ?OnComplete:Void->Void):FlxSound
	{
		...
		#if FLX_PITCH
		useTimeScaleToPitch = true;
		pitch = 1;
		#end
		_length =  _sound?.length ?? 0;
		endTime = _length;
		return this;
	}
	...
	{
		_transform.volume = #if FLX_SOUND_SYSTEM FlxG.sound.muted ? 0 : FlxG.sound.volume * #end (group?.volume ?? 1) * getActualVolume();
		if (_channel != null)
	...
	function set_useTimeScaleToPitch(v:Bool):Bool
	{
		if (useTimeScaleToPitch != v)
		{
			useTimeScaleToPitch = v;
			pitch = pitch;
		}
		return v;
	}
