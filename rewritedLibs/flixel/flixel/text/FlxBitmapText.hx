...
class FlxBitmapDrawCharData
{
	...
	{
		var i = new FlxBitmapDrawCharData();
		i.copyFrom(this);
		return i;
	}
	public function copyFrom(i)
	{
		charIndex = i.charIndex;
		matrix.copyFrom(i.matrix);
		colorTransform.__copyFrom(i.colorTransform);
		useColorTransform = i.useColorTransform;
	}
}

// TODO: Fix memory leak on more than one line
...
class FlxBitmapText extends FlxSprite
{
	 ...
	 * @since 5.6.2-TWIST-PSYCH
	 */
	public var onPreUpdateTextDraw(get, never):FlxTypedSignal<Bool->Void>;
	/**
	 * @since 5.6.2-TWIST-PSYCH
	 */
	public var onPostUpdateTextDraw(get, never):FlxTypedSignal<Bool->Void>;
	var _onPreUpdateTextDraw:FlxTypedSignal<Bool->Void> = null;
	var _onPostUpdateTextDraw:FlxTypedSignal<Bool->Void> = null;
	public static final TEXT_DATA_LENGTH:Int = 3;
	public var textData(default, null):Array<Float>;
	public var textDrawDataLength:Int = 0;
	public var textDrawData(default, null):Array<FlxBitmapDrawCharData>;
	public var borderDrawDataLength:Int = 0;
	public var borderDrawData(default, null):Array<FlxBitmapDrawCharData>;
	...
	override public function destroy():Void
	{
		...
		if (_onPreUpdateTextDraw != null)
		{
			_onPreUpdateTextDraw.destroy();
			_onPreUpdateTextDraw = null;
		}
		if (_onPostUpdateTextDraw != null)
		{
			_onPostUpdateTextDraw.destroy();
			_onPostUpdateTextDraw = null;
		}
	...
	function checkPendingChanges(useTiles:Bool = false):Void
	{
		...
		{
			if (_onPreUpdateTextDraw != null)
				_onPreUpdateTextDraw.dispatch(useTiles);
			updatePixels(useTiles);
			if (_onPostUpdateTextDraw != null)
				_onPostUpdateTextDraw.dispatch(useTiles);
		}
	}
	...
	override public function draw():Void
	{
		...
		else
		{
			...
			for (camera in cameras)
			{
				...
				drawItem = camera.startQuadBatch(font.parent, true, colorTransform != null && colorTransform.hasRGBAOffsets(), blend, antialiasing, shader);

				var struct:FlxBitmapDrawCharData;
				for (j in 0...borderDrawDataLength)
				{
					struct = borderDrawData[j];
					currFrame = font.getCharFrame(struct.charIndex);

					if (clipRect != null)
					{
						clippedFrameRect.copyFrom(clipRect).offset(-struct.matrix.tx, -struct.matrix.ty);
						currFrame = currFrame.clipTo(clippedFrameRect);
					}

					currFrame.prepareMatrix(_matrix);
					_matrix.concat(struct.matrix);
					...
					_matrix.translate(_point.x + ox, _point.y + oy);
					inline _colorParams.setMultipliers(borderRed, borderGreen, borderBlue, bAlpha);
					inline _colorParams.setOffsets(0, 0, 0, 0);
					if (struct.useColorTransform)
						_colorParams.concat(struct.colorTransform);
					drawItem.addQuad(currFrame, _matrix, _colorParams);
				}

				for (j in 0...textDrawDataLength)
				{
					struct = textDrawData[j];
					currFrame = font.getCharFrame(struct.charIndex);
					...
					_matrix.translate(_point.x + ox, _point.y + oy);
					inline _colorParams.setMultipliers(textRed, textGreen, textBlue, tAlpha);
					inline _colorParams.setOffsets(0, 0, 0, 0);
					if (struct.useColorTransform)
						_colorParams.concat(struct.colorTransform);
					drawItem.addQuad(currFrame, _matrix, _colorParams);
	...
	function updateText():Void
	{
		...
		if (!multiLine && _lines.length > 0)
		{
			_lines.splice(1, _lines.length);
		}
	...
	public function getStringWidth(str:String):Int
	{
		...
		for (c in 0...lineLength)
		{
			...
			else if (font.charExists(charCode))
			{
				if (c == lineLength - 1)
				{
					charWidth = Std.int(font.getCharFrame(charCode).sourceSize.x);
				}
				else
				{
	...
	function cutLines():Void
	{
		...
		for (i in 0..._lines.length)
		{
			...
			for (c in 0...line.uLength())
			{
				switch (line.uCharCodeAt(c))
				{
	...
	function splitLineIntoWords(line:String, words:Array<String>):Void
	{
		...
		while (c < lineLength)
		{
			...
			else if (charCode == '-'.code)
			{
				...
				{
					// words.push(word + new UnicodeBuffer(String.fromCharCode(charCode)).toString());
					words.push(word + '-');
				}
	...
	function wrapLineByWord(words:Array<UnicodeString>, lines:Array<UnicodeString>, wordSplit:WordSplitConditions):Void
	{
		...
		while (i < words.length)
		{
			...
			switch (wordSplit)
			{
				case LINE_WIDTH if(startX + wordWidth > maxLineWidth): // wordFitsLine
					addWordByChars();
		...
		for (word in words)
		{
			for (c in 0...word.length)
			{
				...
				else // the char cannot fit on the current line
				{
					...
					{
						if (line.length > 0) // new line isn't empty so we should add it to sublines array and start another one
							lines.push(line);
	...
	function updatePixels(useTiles:Bool = false):Void
	{
		...
		else
		{
			...
			{
				textDrawDataLength = 0;
				borderDrawDataLength = 0;
		...
		{
			case SHADOW:
				for (iterY in 1...iterationsY + 1)
				{
					for (iterX in 1...iterationsX + 1)
					{
						drawText(deltaX * iterX, deltaY * iterY, isFront, bitmap, useTiles);
					}
				}
			case OUTLINE:
	...
	function tileText(posX:Int, posY:Int, isFront:Bool = true):Void
	{
		...
		var data:Array<FlxBitmapDrawCharData> = isFront ? textDrawData : borderDrawData;
		var len:Int = isFront ? textDrawDataLength : borderDrawDataLength;

		var textPos:Int;
		var textLen:Int = Std.int(textData.length / TEXT_DATA_LENGTH);
		var rect:FlxRect = clipRect != null ? FlxRect.get() : null;
		var extraPosX:Float;
		var extraPosY:Float;
		var struct:FlxBitmapDrawCharData;
		...
		for (i in 0...textLen)
		{
			...
			if (rect != null)
			{
				if (struct != null)
				{
					extraPosX = struct.matrix.tx;
					extraPosY = struct.matrix.ty;
				}
				else
				{
					extraPosX =	extraPosY = 0.0;
				}
				rect.copyFrom(clipRect).offset(-textData[textPos + 1] - posX - extraPosX, -textData[textPos + 2] - posY - extraPosY);
				if (font.getCharFrame(Std.int(textData[textPos])).clipTo(rect).type == FlxFrameType.EMPTY)
					continue;
			}

			if (struct == null)
			{
				data[len] = struct = new FlxBitmapDrawCharData();
			}
			else
			{
				struct.reset();
			}
			struct.charIndex = Std.int(textData[textPos]);
			struct.matrix.tx = textData[textPos + 1];
			struct.matrix.ty = textData[textPos + 2];
			len++;
		}

		if (isFront)
			textDrawDataLength = len;
		else
			borderDrawDataLength = len;
	...
	function get_onPreUpdateTextDraw()
	{
		if (_onPreUpdateTextDraw == null)
		{
			_onPreUpdateTextDraw = new FlxTypedSignal<Bool->Void>();
		}
		return _onPreUpdateTextDraw;
	}
	function get_onPostUpdateTextDraw()
	{
		if (_onPostUpdateTextDraw == null)
		{
			_onPostUpdateTextDraw = new FlxTypedSignal<Bool->Void>();
		}
		return _onPostUpdateTextDraw;
	}
	...
	function get_textWidth():Int
	{
		...
		_linesWidth ??= [];
		_linesWidth.resize(_lines.length);
