...
class FlxCamera extends FlxBasic
{
	 ...
	 */
	public var origin:FlxPoint = FlxPoint.get();
	...
	public var viewScaleX(default, null):Float;
	public var viewScaleY(default, null):Float;
	...
	@:noCompletion
	#if FLX_RENDER_TRIANGLE inline #end
	public function startQuadBatch(graphic:FlxGraphic, colored:Bool, hasColorOffsets:Bool = false, ?blend:BlendMode, smooth:Bool = false, ?shader:FlxShader)
	{
		...
		var itemToReturn:FlxDrawItem = _storageTilesHead;
			...
			?shader:FlxShader):FlxDrawTrianglesItem
	{
		if (_currentDrawItem != null
			...
			// && _headTriangles.blending == FlxDrawBaseItem.blendToInt(blend)
			#if !flash
	...
	public function getNewDrawTrianglesItem(graphic:FlxGraphic, smoothing:Bool = false, isColored:Bool = false, ?blend:BlendMode, ?hasColorOffsets:Bool,
			?shader:FlxShader):FlxDrawTrianglesItem
	{
		var itemToReturn:FlxDrawTrianglesItem = _storageTrianglesHead;

		if (itemToReturn != null)
		{
			var newHead:FlxDrawTrianglesItem = itemToReturn.nextTyped;
			itemToReturn.reset();
		...
		// itemToReturn.blending = FlxDrawBaseItem.blendToInt(blend);
		itemToReturn.blend = blend;
		#if !flash
		itemToReturn.hasColorOffsets = hasColorOffsets;
		itemToReturn.shader = shader;
		...
		*/
		{
			(
				#if FLX_RENDER_TRIANGLE
				startTrianglesBatch(frame.parent, smoothing,
								transform != null && transform.hasRGBMultipliers(),
							blend)
				#else
				startQuadBatch(frame.parent,
								transform != null && transform.hasRGBMultipliers(),
								transform != null && transform.hasRGBAOffsets(),
							blend, smoothing, shader)
				#end
			).addQuad(frame, matrix, transform);
		}
	}
		...
		*/
		{
			...
			(
				...
				startQuadBatch(frame.parent,
							...
							blend, smoothing, shader)
				#end
			).addQuad(frame, _helperMatrix, transform);
		}
	}
		...
		*/
		{
			...
			(
				#if !flash
				startTrianglesBatch(graphic, smoothing, isColored, blend,
						isColored || (transform != null && transform.hasRGBAOffsets()),
						shader)
				#else
				startTrianglesBatch(graphic, smoothing, isColored, blend)
				#end
			).addTriangles(vertices, indices, uvtData, colors, position, _bounds);
		}
	}
	...
	function transformObject(object:DisplayObject):DisplayObject
	{
		...
		object.x -= origin.x * (scaleX - initialZoom) * FlxG.scaleMode.scale.x;
		object.y -= origin.y * (scaleY - initialZoom) * FlxG.scaleMode.scale.y;
	...
	function updateShake(elapsed:Float):Void
	{
		if (_fxShakeDuration > 0)
		{
			...
			else
			{
				...
				if (_fxShakeAxes.x)
				{
					var shakePixels:Float = FlxG.random.float(-1, 1) * _fxShakeIntensity * width;
					if (pixelPerfect)
						shakePixels = Math.fround(shakePixels);
				...
				if (_fxShakeAxes.y)
				{
					var shakePixels:Float = FlxG.random.float(-1, 1) * _fxShakeIntensity * height;
					if (pixelPerfect)
						shakePixels = Math.fround(shakePixels);
	...
	function updateInternalSpritePositions():Void
	{
		...
		// else
		{
			...
			canvas.x = -origin.x * (scaleX - initialZoom) * FlxG.scaleMode.scale.x;
			canvas.y = -origin.y * (scaleY - initialZoom) * FlxG.scaleMode.scale.y;
	...
	public function getFactorOrigin(?point:FlxPoint)
	{
		point ??= FlxPoint.get();
		return point.set(origin.x / width, origin.y / height);
	}
	...
	public function fill(Color:FlxColor, BlendAlpha:Bool = true, FxAlpha:Float = 1.0, ?graphics:Graphics):Void
	{
		...
		*/
		...
		graphics.drawRect(viewMarginLeft - 1, viewMarginTop - 1, viewWidth + 2, viewHeight + 2);
		graphics.endFill();
	}
	...
	function drawFX():Void
	{
		...
		if (_fxFlashAlpha > 0.0)
		{
			...
			if (alphaComponent <= 0)
				alphaComponent = 1.0;
			alphaComponent *= _fxFlashAlpha;

			// if (FlxG.renderBlit)
			// {
			// 	fill((Std.int(alphaComponent) << 24) + _fxFlashColor.rgb);
			// }
			// else
			{
				fill(_fxFlashColor.rgb, true, alphaComponent, canvas.graphics);
			}
		}
		...
		if (_fxFadeAlpha > 0.0)
		{
			alphaComponent = _fxFadeColor.alphaFloat;
			if (alphaComponent <= 0)
				alphaComponent = 1.0;
			alphaComponent *= _fxFadeAlpha;
			// if (FlxG.renderBlit)
			// {
			// 	fill((Std.int(alphaComponent) << 24) + _fxFadeColor.rgb);
			// }
			// else
			{
				fill(_fxFadeColor.rgb, true, alphaComponent, canvas.graphics);
			}
		}
	}
	...
	public inline function containsPoint(point:FlxPoint, width:Float = 0, height:Float = 0):Bool
		return __containsPoint(point, width, height);

	@:noCompletion function __containsPoint(point:FlxPoint, width:Float = 0, height:Float = 0):Bool
	{
	...
	{
		return __containsRect(rect);
	}

	@:noCompletion function __containsRect(rect:FlxRect):Bool
	{
	...
	function set_alpha(Alpha:Float):Float
	{
		...
		// else
		{
			canvas.alpha = color.alphaFloat * Alpha;
		}
		return Alpha;
	}
	...
	function set_color(Color:FlxColor):FlxColor
	{
		...
		var colorTransform:ColorTransform = this.colorTransform;
		if (colorTransform == null) return Color;

		colorTransform.redMultiplier = color.redFloat;
		colorTransform.greenMultiplier = color.greenFloat;
		colorTransform.blueMultiplier = color.blueFloat;
		colorTransform.alphaMultiplier = color.alphaFloat * alpha;
	...
	{
		var dispObj = FlxG.renderBlit ? _flashBitmap : canvas;
		return dispObj?.transform.colorTransform;
	}

	function set_colorTransform(i:ColorTransform):ColorTransform
	{
		var dispObj = FlxG.renderBlit ? _flashBitmap : canvas;
		if (dispObj != null)
			dispObj.transform.colorTransform = i;
		return i;
	}
	...
	{
		viewScaleX = (scaleX - initialZoom) / scaleX;
		viewMarginX = origin.x * viewScaleX;
	}

	inline function calcMarginY():Void
	{
		viewScaleY = (scaleY - initialZoom) / scaleY;
		viewMarginY = origin.y * viewScaleY;
	}
	...
	function get_viewMarginLeft():Float
	{
		return viewMarginX;
	}

	function get_viewMarginTop():Float
	{
		return viewMarginY;
	}
	...
	{
		return width - width * viewScaleX;
	}

	inline function get_viewHeight():Float
	{
		return height - height * viewScaleY;
	}
