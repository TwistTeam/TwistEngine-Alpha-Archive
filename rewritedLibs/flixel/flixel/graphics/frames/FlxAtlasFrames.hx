 ...
 */
@:access(Xml)
@:access(flixel.math.FlxBasePoint)
class FlxAtlasFrames extends FlxFramesCollection
{
	static function createFlxRect(X:Float = 0, Y:Float = 0, W:Float = 0, H:Float = 0)
	{
		#if target.threaded
		return (FlxBasePoint._mutex != null ? FlxRect.new : FlxRect.get)(X, Y, W, H);
		#else
	...
	static function texturePackerHelper(frameName:String, frameData:TexturePackerAtlasFrame, frames:FlxAtlasFrames, useFrameDuration = false):Void
	{
		...
		var frameRect:FlxRect = createFlxRect(frame.x, frame.y, frame.w, frame.h);
		if (rotated)
		{
			frameRect.set(frame.x, frame.y, frame.h, frame.w);
	...
	public static function fromLibGdx(source:FlxGraphicAsset, description:String):FlxAtlasFrames
	{
		...
		for (i in 0...numImages)
		{
			...
			var rect = createFlxRect(imageX, imageY, imageWidth, imageHeight);
	...
	public static function fromSparrow(source:FlxGraphicAsset, xml:FlxXmlAsset):FlxAtlasFrames
	{
		...
		var data:Array<Xml> = [
			for (child in xml.getXml().firstElement().children)
				if (child.nodeType == Element && child.nodeName == "SubTexture")
					child
		];
		if (data.length == 0)
		{
		...
		var isV1:Bool = data[0].exists("w") || data[0].exists("h");

		var i:Int = 0;
		var _frame:FlxRect = createFlxRect();
		var name:String;
		var trimmed:Bool;
		var rotated:Bool;
		var rect:FlxRect;
		var texture:Map<String, String>;
		var sourceSize:FlxPoint;

		while (i < data.length)
		{
			texture = data[i++].attributeMap;

			if (isV1)
			{
				texture.set("width", texture.get("w"));
				texture.remove("w");
				texture.set("height", texture.get("h"));
				texture.remove("h");
			}

			name = texture.get("name");
			trimmed = texture.exists("frameX");
			rotated = texture.get("rotated") == "true";

			rect = createFlxRect(
				Std.parseFloat(texture.get("x")), Std.parseFloat(texture.get("y")),
				Std.parseFloat(texture.get("width")), Std.parseFloat(texture.get("height"))
			);
			...
			{
				_frame.set(0, 0, rect.width, rect.height);
			}
			...
			if (rect.isEmpty)
			{
				...
				var frame = frames.addEmptyFrame(_frame);

				frame.name = name;
				frame.offset.set(-_frame.x, -_frame.y);

				continue;
			}

			sourceSize = FlxPoint.get(_frame.width, _frame.height);
			if (rotated && !trimmed)
				sourceSize.set(_frame.height, _frame.width);

			frames.addAtlasFrame(rect, sourceSize,
				FlxPoint.get(-_frame.x, -_frame.y),
				name,
				rotated ? FlxFrameAngle.ANGLE_NEG_90 : FlxFrameAngle.ANGLE_0,
				texture.get("flipX") == "true",
				texture.get("flipY") == "true"
			);
		}
		_frame.put();
	...
	public static function fromTexturePackerXml(source:FlxGraphicAsset, xml:FlxXmlAsset):FlxAtlasFrames
	{
		...
		for (sprite in data.firstElement().elements())
		{
			...
			var rect = createFlxRect(Std.parseInt(sprite.get("x")), Std.parseInt(sprite.get("y")), Std.parseInt(sprite.get("w")), Std.parseInt(sprite.get("h")));
	...
	public static function fromSpriteSheetPacker(Source:FlxGraphicAsset, Description:String):FlxAtlasFrames
	{
		...
		for (i in 0...lines.length)
		{
			...
			var rect = createFlxRect(Std.parseInt(currImageRegion[0]), Std.parseInt(currImageRegion[1]), Std.parseInt(currImageRegion[2]),
