...
class NativeAudioSource
{
	...
	public function getCurrentTime():Int
	{
		...
		else if (handle != null)
		{
			...
			if (stream)
			{
				time = Std.int(bufferTimeBlocks[0] * 1000) + Std.int(AL.getSourcef(handle, AL.SEC_OFFSET) * 1000) - parent.offset;
			}
			else
			{
				...
				time = Std.int(totalSeconds * ratio * 1000) - parent.offset;

				// time = Std.int (AL.getSourcef (handle, AL.SEC_OFFSET) * 1000) - parent.offset;
			}
			if (time < 0) return 0;
			return time;
		}
	...
	public function setCurrentTime(value:Int):Int
	{
		...
		if (handle != null)
		{
			if (stream)
			{
				...
				// if (playing && AL.getSourcei(alSource, AL.SOURCE_STATE) != AL.PLAYING) AL.sourcePlay(handle);
