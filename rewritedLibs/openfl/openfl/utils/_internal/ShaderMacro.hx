...
class ShaderMacro
{
	...
	private static function processGLSLText(source:String, glVersion:String, isFragment:Bool)
	{
		...
		static var attributeKeyword:EReg = ~/attribute ([A-Za-z0-9]+) ([A-Za-z0-9_]+)/g; // g to match all
		static var varyingKeyword:EReg = ~/varying ([A-Za-z0-9]+) ([A-Za-z0-9_]+)/g; // g to match all

		static var texture2DKeyword:EReg = ~/texture2D/g;
		static var glFragColorKeyword:EReg = ~/gl_FragColor/g;
		...
		switch (glVersion)
		{
			...
			case "400", "410", "420", "430", "440", "450", "460":
				return processGLSLText(source, "330", isFragment);
			...
			isFragment:Bool):Array<{name:String, behavior:String}>
	{
		...
		switch (glVersion)
		{
			...
			case "300 es", "310 es", "320 es", "330":
				...
				{
					if (extension.name == "GL_ARB_separate_shader_objects" || extension.name == "GL_EXT_separate_shader_objects")
					{
						#if linux
						glExtensions.push({name: "GL_EXT_separate_shader_objects", behavior: "require"});
						#else
						glExtensions.push({name: "GL_ARB_separate_shader_objects", behavior: "require"});
						#end
						break;
					}
				}
	...
	private static function processFields(source:String, storageType:String, fields:Array<Field>, pos:Position):Void
	{
		...
		var lastMatch = 0, position, field:Field, name, type;

		var regex = new EReg(storageType + " ([A-Za-z0-9]+) ([A-Za-z0-9_]+)", "");
