...
@:final class Context3D extends EventDispatcher
{
	...
	**/
	...
	@:noCompletion private function new(stage:Stage, contextState:Context3DState = null, stage3D:Stage3D = null)
	{
		...
		{
			__driverInfo = "OpenGL Vendor=" + gl.getParameter(gl.VENDOR) +
			...
			wantsBestResolutionOnBrowserZoom:Bool = false):Void
	{
		...
		else
		{
			if (__backBufferTexture == null || backBufferWidth != width || backBufferHeight != height)
			{
				...
				#end
				__stage3D.__vertexBuffer.uploadFromVector(new Vector<Float>([
				...
				if (__stage3D.__indexBuffer == null)
				{
					...
					__stage3D.__indexBuffer.uploadFromVector(new Vector<UInt>([0, 1, 2, 2, 1, 3]), 0, 6);
				}
			}
	...
	**/
	public function drawTriangles(indexBuffer:IndexBuffer3D, firstIndex:Int = 0, numTriangles:Int = -1):Void
	{
		...
		{
			__state.renderToTexture.__dirtyUpdateSampler = true;
		}
		...
		gl.drawElements(gl.TRIANGLES, (numTriangles == -1 ? indexBuffer.__numIndices : numTriangles * 3), gl.UNSIGNED_SHORT, firstIndex * 2);
	}
	...
	**/
	...
	/**
		...
		calling `dispose()` or because the underlying rendering hardware has been lost.
	**/
	public function setSamplerStateAt(sampler:Int, wrap:Context3DWrapMode, filter:Context3DTextureFilter, mipfilter:Context3DMipFilter, lodBias:Float):Void
	{
		...
		if (state == null)
		{
			...
			__state.samplerStates[sampler] = state;
		}
		state.wrap = wrap;
		state.filter = filter;
		state.mipfilter = mipfilter;
		state.lodBias = lodBias;
	}
	...
	**/
	...
	@:noCompletion private function __flushGLScissor():Void
	{
		...
		else
		{
			...
			{
				scissorY = Std.int(__stage.window.height * __stage.window.scale) - scissorHeight - scissorY;
			}
	...
	@:noCompletion private function __flushGLTextures():Void
	{
		...
		for (i in 0...__state.textures.length)
		{
			...
			if (samplerState == null)
			{
				...
				__state.samplerStates[i] = samplerState;
			}
	...
	@:noCompletion private function __flushGLViewport():Void
	{
		...
		if (__state.renderToTexture == null)
		{
			if (__stage.context3D == this)
			{
				...
				{
					scaledBackBufferWidth = Std.int(scaledBackBufferWidth * __stage.window.scale);
					scaledBackBufferHeight = Std.int(scaledBackBufferHeight * __stage.window.scale);
				}
				#end
	...
	@:noCompletion private function _setGLMultiSample(enable:Bool):Void
	{
	...
	@:noCompletion private function get_totalGPUMemory():Int
	{
		if (__glMemoryCurrentAvailable != -1)
		{
			...
			if (total > 0)
			{
				return Std.int(total - gl.getParameter(__glMemoryCurrentAvailable)) * 1024;
			}
		}
		return 0;
	}
