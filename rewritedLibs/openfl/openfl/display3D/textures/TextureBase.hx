...
class TextureBase extends EventDispatcher
{
	...
	@:noCompletion private static var __log2:Float = Math.log(2);

	@:noCompletion private var __dirtyUpdateSampler:Bool = false;
	...
	@:noCompletion private var __maxLodBias:Int = 1000;
	...
	**/
	...
	public function generateMipmap()
	{
		...
		{
			var gl = __context.gl;
			var prevID = __context.__contextState.__currentGLTexture2D;
			__context.__bindGLTexture2D(__textureID);
			gl.generateMipmap(__textureTarget);
			// trace(gl.getTexParameter(__textureTarget, untyped gl.TEXTURE_MIN_LOD));
			// trace(gl.getTexParameter(__textureTarget, untyped gl.TEXTURE_MAX_LOD));
			// trace(gl.getTexParameter(__textureTarget, untyped gl.TEXTURE_BASE_LEVEL));
			// trace(gl.getTexParameter(__textureTarget, untyped gl.TEXTURE_MAX_LEVEL));
			// trace(gl.getTexParameter(__textureTarget, untyped gl.MAX_TEXTURE_LOD_BIAS));
			__context.__bindGLTexture2D(prevID);
		}
		#end
	}

	@:noCompletion private function __calculateMaxLodBias():Void
	{
		if (__width > 0 && __height > 0)
		{
			__maxLodBias = Math.floor(Math.pow(Math.log(__width > __height ? __width : __height) / __log2, 2 / 3)) + 1;
		}
		else
		{
			__maxLodBias = 1000;
	...
	{
		if (!state.equals(__samplerState) || __dirtyUpdateSampler)
		{
			__dirtyUpdateSampler = false;
			var gl = __context.gl;
			...
			#if !web
			gl.texParameterf(__textureTarget, untyped gl.TEXTURE_MAX_LEVEL, __maxLodBias);
