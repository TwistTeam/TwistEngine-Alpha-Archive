	...
	**/
	public function stop():Void
	{
		...
		if (__alSource != null)
		{
			...
			__alBuffers.resize(0);
			__emptyBuffers.resize(0);
			__alBuffers = null;
			__emptyBuffers = null;
			__alSource = null;
		}
		...
		if (alAudioContext != null)
		{
			...
			else
			{
				...
				for (a in 0...__numberOfBuffers)
				{
					...
					alAudioContext.bufferData(__alBuffers[a], alAudioContext.FORMAT_STEREO16, __bufferView, bufferSize * 4, __sound.sampleRate);
				}
	...
	private function watchBuffers(i:Int):Void
	{
		...
		else if (hasSampleData && alAudioContext.getSourcei(__alSource, alAudioContext.SOURCE_STATE) != alAudioContext.PLAYING)
		{
			alAudioContext.sourcePlay(__alSource);
		}
	}
