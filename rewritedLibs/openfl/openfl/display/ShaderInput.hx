...
import openfl.display3D.UniformLocation;
...
@:final class ShaderInput<T> /*implements Dynamic*/
{
	...
	**/
	@SuppressWarnings("checkstyle:Dynamic") public var index(default, null):UniformLocation;
	...
	**/
	...
	/**
		The lod bias to use when binding this shader input.
	...
	**/
	...
	@:noCompletion private function __updateGL(context:Context3D, id:Int, overrideInput:T = null, overrideFilter:Context3DTextureFilter = null,
			overrideMipFilter:Context3DMipFilter = null, overrideWrap:Context3DWrapMode = null,
			overrideLodBias:Float = null):Void
	{
		// var gl = context.gl;
		overrideInput ??= this.input;

		if (overrideInput != null)
		{
			overrideWrap ??= this.wrap;
			overrideFilter ??= this.filter;
			overrideMipFilter ??= this.mipFilter;
			overrideLodBias ??= this.lodBias;
			// TODO: Improve for other input types? Use an interface perhaps

			context.setTextureAt(id, cast (overrideInput, BitmapData).getTexture(context));
			context.setSamplerStateAt(id, overrideWrap, overrideFilter, overrideMipFilter, overrideLodBias);
		}
		else
		{
			context.setTextureAt(id, null);
