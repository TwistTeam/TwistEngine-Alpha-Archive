...
@:final class ShaderParameter<T> /*implements Dynamic*/
{
	...
	**/
	...
	@:noCompletion private function __updateGL(context:Context3D, overrideValue:Array<T> = null):Void
	{
		...
		var boolValue:Array<Bool> = __isBool ? cast overrideValue : null;
		var floatValue:Array<Float> = __isFloat ? cast overrideValue : null;
		var intValue:Array<Int> = __isInt ? cast overrideValue : null;

		if (__isUniform)
		{
			if (overrideValue != null && overrideValue.length >= __length)
			{
				switch (type)
				{
					case BOOL:
					...
					case MATRIX2X2:
						...
						// }
						__uniformMatrix.set(floatValue);
		...
		{
			if (!__useArray && (overrideValue == null || overrideValue.length == __length))
			{
				for (i in 0...__arrayLength)
				{
					gl.disableVertexAttribArray(index + i);
