	...
	public static function render(textField:TextField, renderer:CairoRenderer, transform:Matrix):Void
	{
		...
		if (textEngine.text != null && textEngine.text != "")
		{
			...
			for (group in textEngine.layoutGroups)
			{
				...
				{
					if (textEngine.__cairoFont == null || textEngine.__font != font)
					{
					...
					glyphs.resize(group.positions.length);
					x = group.offsetX + scrollX - bounds.x;
					y = group.offsetY + group.ascent + scrollY - bounds.y;

					var totalLength:Int = 0;
					var _glyph:CairoGlyph;
					for (position in group.positions)
					{
						if (position == null || position.glyph == 0) continue;
						_glyph = glyphs[totalLength];
						if (_glyph == null)
						{
							glyphs[totalLength] = _glyph = new CairoGlyph(0);
						}

						_glyph.index = position.glyph;
						_glyph.x = x + position.offset.x + 0.5;
						_glyph.y = y - position.offset.y + 0.5;

						x += position.advance.x;
						y -= position.advance.y;
						totalLength++;
					}
					glyphs.resize(totalLength);
