...
class DisplayObjectRenderer extends EventDispatcher
{
	...
	@:noCompletion private function __dispose_BitmapData<T:BitmapData>(bitmapData:T):T
	{
		if (bitmapData != null)
		{
	...
	@:noCompletion private function __updateCacheBitmap(displayObject:DisplayObject, force:Bool):Bool
	{
			...
			|| (renderer.__type == OPENGL && displayObject.scale9Grid != null))
		{
			...
			if (updateTransform || needRender)
			{
				...
				offsetX = rect.x > 0 ? Math.fceil(rect.x) : Math.ffloor(rect.x);
				offsetY = rect.y > 0 ? Math.fceil(rect.y) : Math.ffloor(rect.y);
			...
			if (needRender)
			{
				...
				if (filterWidth >= 0.5 && filterHeight >= 0.5)
				{
					...
					if (displayObject.__cacheBitmapData == null
						...
						|| bitmapHeight > displayObject.__cacheBitmapData.height)
					{
						__dispose_BitmapData(displayObject.__cacheBitmapData);
						displayObject.__cacheBitmapData = new BitmapData(bitmapWidth, bitmapHeight, true, bitmapColor);

						__dispose_BitmapData(displayObject.__cacheBitmap?.__bitmapData);
				...
				else
				{
					...
					displayObject.__cacheBitmapData = __dispose_BitmapData(displayObject.__cacheBitmapData);
					displayObject.__cacheBitmapData2 = __dispose_BitmapData(displayObject.__cacheBitmapData2);
					displayObject.__cacheBitmapData3 = __dispose_BitmapData(displayObject.__cacheBitmapData3);
					displayObject.__cacheBitmapRenderer = null;
			...
			if (needRender)
			{
				...
				if (displayObject.__cacheBitmapRenderer.__type == OPENGL)
				{
					...
					if (hasFilters)
					{
						...
						{
							__dispose_BitmapData(displayObject.__cacheBitmapData2);
						...
						if (needCopyOfOriginal)
						{
							...
							{
								__dispose_BitmapData(displayObject.__cacheBitmapData3);
