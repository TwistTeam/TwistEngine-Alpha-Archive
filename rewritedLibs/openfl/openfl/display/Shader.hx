...
class Shader
{
	...
	**/
	...
	/**
	 * Allows safe assignment of uniform variables via the source resource at compile time.
	 */
	public var autoSetUniforms:Bool = false;
	...
	**/
	...
	@:noCompletion private function __processGLData(source:String, storageType:String):String
	{
		var position, name, type;

		var regex = new EReg('$storageType\\s+([A-Za-z0-9]+)\\s+([A-Za-z0-9_]+)', "");
		static var variablesRegex = new EReg("\\s*=\\s*((?:[iudb]?vec[2-4]|mat[2-4](?:x[2-4])?)\\((.*)\\)|(true)|(-?\\s*\\d+.\\d*)|(-?\\s*\\d*.\\d+)|(-?\\s*\\d+))\\s*;","i"); // todo: support for logical operations
			...
			else if (!Reflect.hasField(__data, name) || Reflect.field(__data, name) == null)
			{
				...
				switch (parameterType)
				{
					case BOOL, BOOL2, BOOL3, BOOL4:
						...
						if (isUniform && autoSetUniforms)
						{
							if (variablesRegex.matchSub(source, lastMatch) && variablesRegex.matchedPos().pos == lastMatch)
							{
								if (length == 1)
								{
									parameter.value = [variablesRegex.matched(3) != null];
								}
								else
								{
									var vecValues = variablesRegex.matched(2);
									if (vecValues == null)
										parameter.value = [for (i in 0...length) false];
									else
									{
										parameter.value = [for (i in vecValues.split(",")) i.indexOf("true") != -1];
										if (parameter.value.length == 1 && 1 != length)
										{
											parameter.value = [for (i in parameter.value) parameter.value[0]];
										}
										else while (parameter.value.length < length)
											parameter.value.push(false);
									}
								}
								var pos = variablesRegex.matchedPos();
								source = source.substring(0, pos.pos) + source.substr(pos.pos + pos.len - 1);
							}
							else
							{
								parameter.value = [for (i in 0...length) false];
					...
					case INT, INT2, INT3, INT4:
						...
						if (isUniform && autoSetUniforms)
						{
							if (variablesRegex.matchSub(source, lastMatch) && variablesRegex.matchedPos().pos == lastMatch)
							{
								if (length == 1)
								{
									var strInt = variablesRegex.matched(6);
									parameter.value = [strInt != null ? Std.parseInt(strInt) : 0];
								}
								else
								{
									var vecValues = variablesRegex.matched(2);
									if (vecValues == null)
										parameter.value = [for (i in 0...length) 0];
									else
									{
										parameter.value = [for (i in vecValues.split(",")) Std.parseInt(i)];
										if (parameter.value.length == 1 && 1 != length)
										{
											parameter.value = [for (i in parameter.value) parameter.value[0]];
										}
										else while (parameter.value.length < length)
											parameter.value.push(0);
									}
								}
								var pos = variablesRegex.matchedPos();
								source = source.substring(0, lastMatch) + source.substr(pos.pos + pos.len - 1);
							}
							else
							{
								parameter.value = [for (i in 0...length) 0];
					...
					default:
						...
						if (isUniform && autoSetUniforms)
						{
							if (variablesRegex.matchSub(source, lastMatch) && variablesRegex.matchedPos().pos == lastMatch)
							{
								...
								if (length == 1)
								{
									var strFloat = variablesRegex.matched(4);
									if (strFloat == null)
									{
										strFloat = variablesRegex.matched(5);
									}
									if (strFloat == null)
									{
										strFloat = variablesRegex.matched(2);
									}
									parameter.value = [strFloat != null ? parseFloat(strFloat) : 0.0];
								}
								else
								{
									var vecValues = variablesRegex.matched(2);
									if (vecValues == null)
										parameter.value = [for (i in 0...length) 0.0];
									else
									{
										parameter.value = [for (i in vecValues.split(",")) parseFloat(i)];
										if (parameter.value.length == 1 && 1 != length)
										{
											parameter.value = [for (i in 0...length) parameter.value[0]];
										}
										else while (parameter.value.length < length)
											parameter.value.push(0.0);
									}
								}
								var pos = variablesRegex.matchedPos();
								source = source.substring(0, lastMatch) + source.substr(pos.pos + pos.len - 1);
	...
	@:noCompletion private function __updateGLFromBuffer(shaderBuffer:ShaderBuffer, bufferOffset:Int):Void
	{
		...
		for (i in 0...shaderBuffer.inputCount)
		{
			...
			if (inputData != null)
				shaderBuffer.inputRefs[i].__updateGL(__context, textureCount++, inputData, shaderBuffer.inputFilter[i], shaderBuffer.inputMipFilter[i], shaderBuffer.inputWrap[i], shaderBuffer.inputLodBias[i]);
		}
		...
		for (i in 0...shaderBuffer.paramCount)
		{
			if (i < boolCount)
			{
				...
				if (shaderBuffer.overrideBoolValues.exists(boolRef.name))
				{
					boolRef.__updateGL(__context, shaderBuffer.overrideBoolValues[boolRef.name]);
				}
				else
				{
			...
			else if (i < boolCount + floatCount)
			{
				...
				if (shaderBuffer.overrideFloatValues.exists(floatRef.name))
				{
					floatRef.__updateGL(__context, shaderBuffer.overrideFloatValues[floatRef.name]);
				}
				else
				{
			...
			else
			{
				...
				if (shaderBuffer.overrideIntValues.exists(intRef.name))
				{
					intRef.__updateGL(__context, cast shaderBuffer.overrideIntValues[intRef.name]);
				}
				else
				{
