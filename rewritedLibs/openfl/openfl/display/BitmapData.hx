...
class BitmapData implements IBitmapDrawable
{
	...
	**/
	...
	public var useCashedDraw:Bool = true;
	...
	**/
	public function clone():BitmapData
	{
		...
		bitmapData.useCashedDraw = useCashedDraw;
	...
	**/
	...
	public function fullDispose():Void
	{
		if (__texture != null)
		{
			__texture.dispose();
	...
	**/
			...
			clipRect:Rectangle = null, smoothing:Bool = false):Void
	{
		...
		if (!readable && Lib.current.stage.context3D != null)
		{
			...
			{
				__renderer = null;
				render = new OpenGLRenderer(Lib.current.stage.context3D, this);
			}
			render.__allowSmoothing = smoothing;
			render.__pixelRatio = #if openfl_disable_hdpi 1 #else Lib.current.stage.window.scale #end;
			render.__overrideBlendMode = blendMode;

			render.__worldTransform = transform;
			render.__worldAlpha = 1 / source.__worldAlpha;
			render.__worldColorTransform = _colorTransform;

			render.__resize(width, height);

			if (clipRect != null)
			{
				render.__pushMaskRect(clipRect, clipMatrix);
			}

			__drawGL(source, render);

			if (clipRect != null)
			{
				render.__popMaskRect();
	...
	**/
	@:dox(hide) public function getVertexBuffer(context:Context3D, scale9Grid:Rectangle = null, targetObject:DisplayObject = null):VertexBuffer3D
	{
					...
					|| __vertexBufferScaleY != targetObject.scaleY)))
		{
			...
			__uvRect.setTo(0, 0, newWidth, newHeight);
			...
			#else
			__uvRect.setTo(0, 0, width, height);
	...
	**/
	...
	{
		if (Std.isOfType(source, DisplayObject)) renderer.__stage = cast(source, DisplayObjectContainer).stage; // ?fix¿
