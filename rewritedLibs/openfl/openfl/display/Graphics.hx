...
@:final class Graphics
{
	...
	@:noCompletion private var __lastShader:Shader;
	...
	@:noCompletion private function new(owner:DisplayObject)
	{
		...
		__shaderBufferPool = new ObjectPool<ShaderBuffer>(ShaderBuffer.new);
	...
	**/
	...
	@:noCompletion private function __update(displayMatrix:Matrix, pixelRatio:Float):Void
	{
		...
		if (pixelRatio > 1.0)
		{
			...
			__worldTransform.tx = Math.fround(tx * pixelRatio) / pixelRatio;
			__worldTransform.ty = Math.fround(ty * pixelRatio) / pixelRatio;
		}
		else
		{
