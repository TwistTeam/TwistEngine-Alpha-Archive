...
class OpenGLRenderer extends DisplayObjectRenderer
{
	...
	**/
	...
	@:noCompletion private function new(context:Context3D, defaultRenderTarget:BitmapData = null)
	{
		...
		__defaultShader = new Context3DMaskShader();
	...
	**/
	public function applyBitmapData(bitmapData:BitmapData, smooth:Bool, repeat:Bool = false):Void
	{
		...
		{
			var textureInput = __currentShader.__bitmap;
			if (textureInput != null)
			{
				textureInput.input = bitmapData;
				textureInput.filter = (smooth && __allowSmoothing) ? LINEAR : NEAREST;
				textureInput.mipFilter = MIPNONE;
				textureInput.wrap = repeat ? REPEAT : CLAMP;
			}

			textureInput = __currentShader.__texture;
			if (textureInput != null)
			{
				textureInput.input = bitmapData;
				textureInput.filter = (smooth && __allowSmoothing) ? LINEAR : NEAREST;
	...
	**/
	...
	{
		if (__currentShader == null) return;
	...
	**/
	...
	{
		if (__currentShader != null && __currentShader.__alpha != null)
		{
			__currentShader.__alpha.__useArray = true;
		}
	}
	...
	**/
	...
	@:noCompletion private function __getMatrix(transform:Matrix, pixelSnapping:PixelSnapping):Array<Float>
	{
		...
		if (pixelSnapping == ALWAYS
			|| (pixelSnapping == AUTO
				&& _matrix.b == 0
				&& _matrix.c == 0
				&& (_matrix.a > 0.999 && _matrix.a < 1.001)
				&& (_matrix.d > 0.999 && _matrix.d < 1.001)))
		{
			_matrix.tx = Math.fround(_matrix.tx);
			_matrix.ty = Math.fround(_matrix.ty);
		}
	...
	{
		if (source == null || shader == null || __defaultRenderTarget == null) return;
		...
		// var shader = __initShader(shader) ??  __defaultShader;
		setShader(shader);
		applyAlpha(1);
		...
		__context3D.drawTriangles(source.getIndexBuffer(__context3D));
	...
	@:noCompletion private function __scissorRect(clipRect:Rectangle = null):Void
	{
		if (clipRect != null)
		{
			var x:Float = Math.ffloor(clipRect.x);
			var y:Float = Math.ffloor(clipRect.y);
			var width:Float = (clipRect.width > 0 ? Math.fceil(clipRect.right) - x : 0);
			var height:Float = (clipRect.height > 0 ? Math.fceil(clipRect.bottom) - y : 0);
			#if !openfl_dpi_aware
			if (__context3D.__backBufferWantsBestResolution)
			{
